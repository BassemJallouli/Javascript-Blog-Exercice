...
class Article {
  ...
  const articleSASS = new Article ( // create new article SASS
     'Intro SASS',
     'Text text text text text'
  )

  blogArticles.splice(3,0, articleSASS); // add the new article SASS

  const blogElement = document.body; // display html

  for ( let article of blogArticles )
  {
    // TODO: Loop through articles and output each of them to the browser.
    article.output(blogElement);
  }
